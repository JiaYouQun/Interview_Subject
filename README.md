...
先说说被引用的情况。　　
1.在一个{}代码块中，这个代码块是引用到了对象。{}代码块执行结束以后，对象的引用消失。　　
2.另一个对象引用该对象作为自己的成员变量。　　
在这里，ARC提供了两种类型来作为一个对象的成员变量，分别是：
Strong,Weak(即assign).形象的表示了对一个成员变量的持有态度。　　
即Strong引用。weak并不进行引用。　　
3.特殊对象block。block本质上可以理解为一个对象，block方法体{}内的对象会被它引用。　　
4.全局变量。当一个对象成为全局变量，那么他将在程序生命周期中被持有。　　
5.静态常量。一个静态常量的对象　　
