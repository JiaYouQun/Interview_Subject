...
内存管理是个老生常谈的问题。iOS的内存管理，即内存应用计数器。原理就是，一块内存如果被至少一个地方引用，就不会被释放。基于这个原理，设计了iOS对象的内存管理。在ARC下，一个对象什么时候被引用，什么时候应用无效呢。
先说说被引用的情况:<br /> 
1.在一个{}代码块中，这个代码块是引用到了对象。{}代码块执行结束以后，对象的引用消失。<br /> 
2.另一个对象引用该对象作为自己的成员变量。在这里，ARC提供了两种类型来作为一个对象的成员变量，分别是:Strong,Weak(即assign).形象的表示了对一个成员变量的持有态度.即Strong引用。weak并不进行引用.<br /> 
